...
##   .. )
##  - attr(*, "problems")=<pointer: 0x000001b854dfb5d0>
```
...
<img src="Spangler_project_02_files/figure-html/project2-bad-drive-time-chart-1.png" alt="A poorly designed scatterplot showing average drive time by average commute distance for Florida counties. The chart uses large opaque points, a red-green color scale, and limited explanation, making the pattern harder to interpret."  />

This chart is a weak version of the drive time and commute distance visualization. The points are large and fully opaque, which makes overlapping counties harder to compare. The red and green color scale is also not very accessible for colorblind readers. The title and axis labels are too vague, and the chart does not include a trend line to help show the overall relationship.
...
<img src="Spangler_project_02_files/figure-html/project2-redesigned-drive-time-chart-1.png" alt="Redesigned scatterplot showing average drive time by average commute distance for Florida counties. Transparent points reduce overlap, point size shows number of routes, and a dashed trend line shows the overall relationship."  />

The redesigned chart improves the same visualization by using clearer labels, transparent points, and a more accessible color choice. Point size still shows the number of routes, while the dashed trend line makes the overall relationship between commute distance and drive time easier to understand. The chart also avoids relying only on color because the main relationship is shown through x-axis and y-axis position.
