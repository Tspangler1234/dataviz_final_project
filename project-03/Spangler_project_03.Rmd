...
cement_strength_plot <- ggplot(
  ...
  geom_point(alpha = 0.75) + scale_color_viridis_d(option = "D", name = "Strength Range") + scale_size_continuous(range = c(1.5, 6), name = "Age") +
  labs(
...
```{r project3-bad-age-line-chart, message=FALSE, warning=FALSE, fig.alt="A poorly designed line chart showing concrete compressive strength by age. The chart connects individual observations even though they are not a continuous time series."}
...
```{r project3-redesigned-age-boxplot, message=FALSE, warning=FALSE, fig.alt="Redesigned boxplot showing concrete compressive strength by curing age. Each boxplot shows the spread of strength values for a different age group."}
ggplot(new_concrete, aes(
  x = factor(Age),
  y = Concrete_compressive_strength,
  fill = factor(Age)
